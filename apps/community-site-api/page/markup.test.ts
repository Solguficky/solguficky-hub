...
describe("html-validate: разметка проходит рекомендованный набор правил", () => {
  ...
  // редактируется.
  //
  // У страницы «Аукцион 2026» находок нет, и строки в таблице у неё тоже нет:
  // пустой состав — это `{}` из `?? {}` ниже. Прежние 55 `unique-landmark`
  // давала разметка заметок — каждая была `<aside>`, то есть ориентиром
  // complementary без имени, и экранный диктор предлагал полсотни одинаковых
  // «дополнительных областей». Заметки стали `<div class="note">`: ориентиром
  // на странице остался один настоящий — боковая колонка.
