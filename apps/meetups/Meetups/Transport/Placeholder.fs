/// Заглушечные ответы контракта. Четыре командные операции уже идут через свои
/// срезы, поэтому модуль обслуживает только два чтения — `GetMeetup` и
/// `ListVisibleMeetups` — и уходит вместе с единым путём чтения со смотрящим.
