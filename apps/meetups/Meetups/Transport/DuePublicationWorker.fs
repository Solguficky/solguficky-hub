...
///
/// Варианта «не настроен» у неё нет, в отличие от публикации из журнала: там порт
/// наружу ещё не написан (PER-209), а здесь всё, что нужно тику, — та же база, в
/// которую сервис уже пишет команды.
