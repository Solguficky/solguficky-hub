...
| `check` | Проверяет наличие `nats` CLI и то, что сгенерированные классы импортируются |
...
Схема, импортирующая соседнюю схему того же модуля, заставляет `protoc` написать импорт от корня модуля — `from identity.v1 import roles_pb2`. Внутри пакета такого имени нет, и модуль перестаёт импортироваться. Скрипт переписывает такие импорты под `nats_tester.generated`, оставляя алиас после `as` нетронутым, и падает, если после переписывания в файлах остался хоть один импорт от корня.
