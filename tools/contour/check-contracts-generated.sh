#!/usr/bin/env sh
# Verify the contour contracts project holds no handwritten code.
#
# Same gate as Meetups and Notifications, on the same grounds (ADR-025): while
# nothing is written by hand there, moving to another generator is replacing one
# project, not a migration. Grpc.Tools writes the generated C# into obj/, so
# under version control the project is exactly its .csproj and nothing else.
#
# This project exists because Identity generates Go only: the contour needs a C#
# client for it, and that client needs an owner that does not depend on test
# code. When a C# service becomes a consumer, the project moves to
# shared/dotnet/ unchanged.
#
...
PROJECT='tests/contour/Contour.Contracts'
ALLOWED="$PROJECT/Contour.Contracts.csproj"
...
if [ -n "$unexpected" ]; then
    ...
    printf '%s\n' "$unexpected" | sed 's/^/  - /' >&2
    echo "Boot logic belongs to Contour.Environment, scenarios to Contour.SmokeTests." >&2
    exit 1
fi
