...
`GLOBAL_ROLE_ADMIN = 1` в схеме задаёт имя для человека и JSON, а на проводе поле несёт только номер. У `repeated GlobalRole global_roles = 2` packable-значения по умолчанию идут в packed-форме: единственный `GLOBAL_ROLE_MEMBER = 3` — это `0x12 0x01 0x03` (тег поля 2 с wire type 2, длина 1, значение), а не unpacked `0x10 0x03`. Unpacked-форму парсер обязан принимать тоже — на этом стоит wire-тест ниже, где поле 2 приходит байтами `0x10 0x63` со значением 99. Список имён схемой не является барьером: добавление значения расширяет словарь, а не формат. Поэтому расширение enum совместимо в обе стороны, если номер не переиспользован и тип поля не изменён.
