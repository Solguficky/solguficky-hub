...
При старте `main` сначала применяет миграции, потом слушает порт. Соединение с базой остаётся открытым на время жизни процесса: его использует `ResolveIdentity`. Повторный старт на той же базе — успех без ошибки: goose сравнивает таблицу `goose_db_version` с файлами и ничего не делает, если версия уже текущая. Строка подключения читается из `IDENTITY_DATABASE_URL` и в лог не попадает — [logging.md](../../standards/observability/logging.md) запрещает connection strings. Telegram user id первого администратора — `IDENTITY_ADMIN_TELEGRAM_USER_ID`; пустое значение означает, что bootstrap выключен.

Идентификаторы профиля и выдачи роли генерирует `uuid.NewV7()` из `github.com/google/uuid`: в стандартной библиотеке Go UUID нет, а PostgreSQL 16 ещё не имеет `uuidv7()`. Это ближайший к [ADR-020](../../decisions/ADR-020-uuidv7-identifiers.md) нативный путь в этом языке.
