...
Часть механики добавлена позже и из другого среза: миграции Notifications применяют вендоренные скрипты Orleans, и на них всплыли правила замены функций и долларового квотирования. Кластер, ради которого эти скрипты существуют, разобран в [orleans/grains-and-cluster.md](../orleans/grains-and-cluster.md).
...
### `CREATE OR REPLACE FUNCTION` меняет тело, но не подпись

Механика всплыла на вендоринге скриптов Orleans в миграции Notifications: оригинал не идемпотентен, и `CREATE FUNCTION` в нём заменён на `CREATE OR REPLACE FUNCTION`, чтобы повторный прогон не падал. Замена работает, но предел у неё уже, чем кажется: `OR REPLACE` переписывает тело и не трогает опознание функции.

Проверено на `postgres:16-alpine`:

```
CREATE OR REPLACE FUNCTION probe(p t.port%TYPE) RETURNS integer ...   -- CREATE FUNCTION
CREATE OR REPLACE FUNCTION probe(p t.port%TYPE) RETURNS integer ...   -- другое тело: CREATE FUNCTION

CREATE OR REPLACE FUNCTION probe(port_arg t.port%TYPE) RETURNS integer ...
-- ERROR:  cannot change name of input parameter "p"
-- HINT:   Use DROP FUNCTION probe(integer) first.

CREATE OR REPLACE FUNCTION probe(p integer) RETURNS bigint ...
-- ERROR:  cannot change return type of existing function
```

Имя входного параметра — часть контракта, потому что по нему вызывают: `probe(p => 1)`. Тип возврата — тоже, от него зависит всё, что уже разобрало вызов этой функции. Меняется и то и другое только через `DROP FUNCTION`.

Следствие для вендоренного скрипта важно тем, **когда** оно срабатывает. Пока подписи совпадают с уже установленными, повторный прогон — дешёвый no-op. Стоит новой версии Orleans переименовать параметр — и миграция упадёт не при первой установке, а при обновлении, то есть только у того, у кого база уже есть; на чистой базе всё зелено. Поэтому условие записано прямо в шапке скрипта: перевендоренный файл обязан начинаться с `DROP FUNCTION IF EXISTS`, если подписи изменились.

### `%TYPE` — якорь на колонку, срабатывающий один раз

Вендорный скрипт объявляет параметры через якорь, чтобы не повторять `varchar(150)` в десятке мест:
...
Ключевой вопрос — когда якорь раскрывается. При создании функции, а не при вызове, и PostgreSQL говорит об этом вслух:

```
NOTICE:  type reference t.port%TYPE converted to integer

ALTER TABLE t ALTER COLUMN port TYPE bigint;
SELECT pg_get_function_arguments('probe'::regproc);   -- p integer
```

Якорь за колонкой не следует: после `ALTER TABLE` функция продолжает принимать `integer`. Для интуиции, воспитанной на рефлексии, это обратное ожидание — запись выглядит как ссылка, а ведёт себя как подстановка на месте. Практический вывод: `%TYPE` экономит правки при написании и ничего не обещает при эволюции схемы. Смена типа колонки требует пересоздания всех функций, которые на неё сослались, и молчание здесь опаснее ошибки.

### Долларовое квотирование делит синтаксис с чужими шаблонизаторами

Тело функции на PL/pgSQL — обычная строковая константа, и чтобы не экранировать в ней кавычки, его оборачивают долларами: `$$ ... $$` либо именованно `$func$ ... $func$`. Имя между долларами произвольно и нужно ровно для вложенности: тело, внутри которого встречается `$$`, оборачивается тегом подлиннее.

На первом прогоне миграций Notifications это столкнулось с подстановкой переменных DbUp, который считает `$func$` своей переменной, и накатка остановилась сообщением `Variable func has no value defined`. Выключается подстановка целиком:

```csharp
.WithVariablesDisabled()
```

Механизм общий и переживёт конкретный инструмент: долларовое квотирование PostgreSQL занимает тот же синтаксис, что множество шаблонизаторов. Прежде чем пропускать SQL с функциями через любой слой, который «умеет переменные», стоит выяснить, не считает ли он доллары своими — иначе отказ приходит из места, которое к SQL отношения не имеет.
...
Отдельно переносится наблюдение про идемпотентность DDL: **она не сводится к `IF NOT EXISTS`.** У таблицы идемпотентность даёт одно слово, у функции — `OR REPLACE`, который покрывает тело и не покрывает подпись, а у строки-справочника — `ON CONFLICT DO UPDATE`, потому что `DO NOTHING` оставил бы устаревшее значение молча. Общее в трёх случаях одно: идемпотентность — свойство пары «скрипт и уже установленное состояние», а не свойство скрипта. Проверяется она поэтому вторым прогоном на непустой базе, и ловушки выстреливают у того, у кого данные уже есть.
...
- [CREATE FUNCTION](https://www.postgresql.org/docs/16/sql-createfunction.html) — сюда за тем, что именно заменяет `OR REPLACE` и почему имена параметров входят в подпись.
- [Dollar-Quoted String Constants](https://www.postgresql.org/docs/16/sql-syntax-lexical.html#SQL-SYNTAX-DOLLAR-QUOTING) — сюда за правилами вложенности и произвольностью тега между долларами.
- [Copying Types: %TYPE](https://www.postgresql.org/docs/16/plpgsql-declarations.html#PLPGSQL-DECLARATION-TYPE) — сюда за моментом, в который якорь раскрывается.
...
**Что произойдёт, если повторно накатить `CREATE OR REPLACE FUNCTION` с переименованным параметром?**
Отказ `cannot change name of input parameter`. Тело заменяется, подпись — нет; для смены подписи нужен `DROP FUNCTION`. Проверяется двумя `CREATE OR REPLACE` подряд над одной функцией в любом psql.

**Изменится ли тип параметра, объявленного через `%TYPE`, после `ALTER TABLE` над колонкой?**
Нет. Якорь раскрывается при создании функции; `pg_get_function_arguments` после `ALTER TABLE` показывает прежний тип.

**Зачем писать `$func$`, если работает `$$`?**
Ради вложенности: тело, содержащее `$$`, иначе закончится раньше времени. Имя между долларами ни на что другое не влияет — и именно поэтому его может перехватить чужая подстановка переменных.
