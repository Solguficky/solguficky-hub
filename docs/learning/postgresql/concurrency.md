...
### Что из этого следует для схемы: выборка по состоянию вместо курсора

`position` в журнале оставлен как порядок обхода: перечислить события в разумном порядке он позволяет. Курсором публикации он служить не может, и в схеме это записано комментарием, а не оставлено на догадку читателя.

Механизм отметки выбран в [PER-164](https://linear.app/anticnvm/issue/per-164): колонка `dispatched_at` в том же журнале плюс частичный индекс по неотправленным ([ADR-035](../../decisions/ADR-035-meetups-dispatch-mark-in-journal.md)).

```sql
ALTER TABLE meetup_events ADD COLUMN IF NOT EXISTS dispatched_at TIMESTAMPTZ;
...
Механика лечения ровно в замене вопроса. Курсор спрашивает «что идёт после номера N» — вопрос о позиции в последовательности, а последовательность и порядок коммитов не совпадают. Отметка спрашивает «что ещё не отправлено» — вопрос о состоянии самой строки, и на него поздний коммит отвечает правильно, когда бы он ни случился. Строка, закоммиченная после того, как читатель ушёл вперёд, просто попадает в следующую выборку: её `dispatched_at` всё ещё `NULL`.

Проверено тем же сценарием из двух сессий, что и потеря выше:

```
A: INSERT -> position 1   (транзакция открыта)
B: INSERT -> position 2, COMMIT

читатель: UPDATE ... WHERE dispatched_at IS NULL RETURNING event_id
          -> вернул только событие B

A: COMMIT

WHERE position > 2          -> пусто          <-- курсор потерял бы A
WHERE dispatched_at IS NULL -> событие A      <-- отметка его находит
```

`position` при этом никуда не делся: он задаёт `ORDER BY` внутри уже отобранного набора. Порядок обхода — его законная работа; несущим состояние он не стал.
...
Лечится это не более умным курсором, а **заменой вопроса**: «что идёт после N» на «что ещё не сделано». Первый вопрос — о позиции в последовательности и потому наследует её порядок; второй — о состоянии самой записи, и его ответ не зависит от того, когда запись стала видимой. Та же замена работает в любой очереди поверх таблицы, независимо от СУБД.
...
**Курсор публикации: отметка в журнале против отдельной таблицы outbox.** Отдельная таблица со своей копией события развела бы историю домена и состояние доставки, и `dispatched_at` не появился бы в журнале вовсе. Отвергнуто замером: копия заняла больше оригинала (24 МБ против 21 МБ на 80 тыс. событий — дублируются и payload, и индексы), а главную цену не убрала. Отметка отправки — не-HOT UPDATE в любом случае, потому что колонка входит в предикат частичного индекса (см. [schema.md](schema.md#частичный-индекс-и-hot-update)); разделение таблиц переносит эту запись в другую таблицу, а не отменяет её. Возражение «журнал перестаёт быть неизменяемым» снято триггерами: неизменяема запись события, подвижна только отметка.

**Курсор публикации: отметка против `pg_snapshot_xmin(pg_current_snapshot())`.** Курсор по порядку коммитов сохранил бы журнал полностью неизменяемым: читатель двигался бы не за максимальным номером, а за границей, ниже которой открытых транзакций уже нет. Отвергнуто по поведению под нагрузкой: эта граница стоит на месте, пока жива самая старая открытая транзакция базы — любая, не только пишущая в журнал. Отставание публикации стало бы функцией чужой нагрузки, а диагностировалось бы как «релей тормозит».

**Курсор публикации: отметка против logical decoding.** CDC даёт настоящий порядок коммитов и не требует ни отметки, ни курсора. Отвергнуто по цене контура: replication slot, удержание WAL до вычитки, отдельный коннектор и его эксплуатация — ради одного сервиса, у которого сходки заводят руками.
...
Тот же сценарий с выборкой по состоянию — строка не теряется:

```mermaid
sequenceDiagram
    participant A as Транзакция A
    participant B as Транзакция B
    participant R as Релей (отметка)
    A->>A: BEGIN, затем INSERT -> position 1
    B->>B: BEGIN, затем INSERT -> position 2
    B->>B: COMMIT
    Note over R: WHERE dispatched_at IS NULL
    R->>R: видит только 2, публикует, ставит отметку
    A->>A: COMMIT
    Note over A,R: строка 1 видима, отметки на ней нет
    R->>R: следующая выборка возвращает строку 1
```
...
**Релей отметил всё, что видел. Транзакция с меньшим номером закоммитилась после этого. Прочитает ли он её в следующий раз?**
Да. Предикат `dispatched_at IS NULL` спрашивает про состояние строки, а не про её позицию: поздний коммит приносит строку без отметки, и она попадает в следующую выборку. Тот же сценарий с `WHERE position > last` вернул бы пусто.

```bash
# держатель: BEGIN; INSERT; (пауза) — и параллельно вторая сессия с INSERT+COMMIT
# релей между ними: UPDATE ... WHERE dispatched_at IS NULL RETURNING event_id
# после COMMIT держателя: WHERE position > 2 -> пусто, WHERE dispatched_at IS NULL -> строка есть
```
