...
Файл объясняет термины и модель мира [RFC-010](../../rfcs/RFC-010-remote-development-and-self-hosting-platform.md) читателю, который не администрировал Linux-сервер. Он не нормирует: требования к хосту живут в RFC, принятое решение — в [ADR-039](../../decisions/ADR-039-single-vps-for-initial-self-hosting.md), порядок работ — в [PER-80](https://linear.app/anticnvm/issue/per-80).
...
**Один VPS, а не два.** Два хоста дали бы настоящую границу между недоверенной разработкой и боевым контуром. Цена — второй жизненный цикл хоста до того, как появится измеренная необходимость. Решение и сигналы пересмотра — в ADR-039.
