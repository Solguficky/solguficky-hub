...
То же для enum `GlobalRole` из сгенерированного файла: значения словаря разобраны по имени, default снова `never`. `never` держит полноту только для известных компилятору значений: неизвестное число из более новой Identity доходит в рантайме, поэтому `default` обязан обработать его безопасно — вернуть `undefined` и отбросить роль, а не превратить её в строку.
