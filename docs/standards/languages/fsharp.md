...
Механическое форматирование норматив не описывает: ширину, раскладку записей и скобок задаёт конфигурация Fantomas в корневом `.editorconfig`. Там же лежит стиль C# — он действует на границе .NET и на AppHost.

Одно правило написания следует из этой конфигурации напрямую: многострочный литерал не оборачивается в вызов функции. Пиши `[ … ] |> set` и `{ … } |> Some`, а не `set [ … ]` и `Some { … }`. В первом случае литерал стоит на своём отступе, а функция читается отдельным шагом конвейера; во втором Fantomas выносит скобки под имя функции и добавляет им лишний уровень отступа, и значение начинает выглядеть хвостом вызова.
