...
Форму имени теста, файла и тестового проекта задаёт [naming.md](naming.md).
...
Один валидный полный образец на доменный тип, вариации — клонированием через `with`. Отличие от нормы и есть суть теста; полный литерал из двадцати полей в каждом тесте её прячет.

```fsharp
module Sample =
    let meetup =
        { Id = MeetupId.parse "0199c3f0-0000-7000-8000-000000000001"
          Title = "F# after hours"
          Visibility = Unlisted
          Capacity = Capacity.create 30 |> Result.defaultWith (fun _ -> failwith "unreachable")
          StartsAt = fixedNow.AddDays 7.0 }

[<Fact>]
let ``Published meetup is visible to a stranger`` () =
    let meetup = { Sample.meetup with Visibility = Public }

    test <@ Visibility.canView Stranger meetup = true @>
```

- Образец валиден целиком. Невалидное значение появляется ровно в том тесте, который проверяет отказ на нём.
- Образец одного файла живёт в модуле `Sample` внутри него; общий для проекта — в `TestUtilities/TestData`, общий для нескольких тест-проектов — в `<Компонент>.TestKit`. Раскладку задаёт [testing-strategy.md](testing-strategy.md).
- Дефолт данных и дефолт зависимости — разные вещи: у образца данных он валидный и молчаливый, у поля `Deps` — падающий, см. ниже.
...
- Правило касается полей, чей вызов сам является наблюдаемым эффектом: чтение хранилища, запись, отправка сообщения, обращение к внешнему сервису. Часы, генератор идентификаторов и логгер получают фиксированное значение: их вызов ничего не наблюдает, и падение на нём ничего не ловит.
