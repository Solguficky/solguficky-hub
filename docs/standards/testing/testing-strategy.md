...
- Имя тест-проекта называет уровень: `<Компонент>.UnitTests`, `<Компонент>.IntegrationTests`, `<Компонент>.E2ETests`. Сквозной E2E, не принадлежащий одному компоненту, — отдельный проект уровня решения. Общие утилиты нескольких тест-проектов — `<Компонент>.TestKit`. Проект без уровня в имени (`<Компонент>.Tests`) не заводится: уровень виден в дереве решения и в фильтрах запуска.
