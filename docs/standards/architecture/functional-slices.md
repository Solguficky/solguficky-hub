...
У gRPC эта строка делится надвое: отображение остаётся в `Api` среза, а endpoint-а у среза нет — транспортный класс один на сервис. Разбор — в разделе [gRPC](#grpc).
...
- Lifetime и disposal ресурсов принадлежат composition root приложения; срез не открывает и не закрывает соединение сам, если этого не требует его транзакция.
...
| transport | HTTP status, gRPC code | `Api` среза | 409, 404, 500; `FailedPrecondition`, `NotFound`, `Internal` |
...
- `DomainError` агрегата не несёт отказа по правам: право действовать не является инвариантом перехода состояния, и в решении о нём состояние не участвует.
- Транспортный словарь внутри ядра запрещён: `Unauthorized`, `Forbidden`, `403` и `PERMISSION_DENIED` в `Domain/` не встречаются. Ядро называет отказ своим словарём, отображает его срез.
- Когда решение о праве принадлежит самому сервису и принимается из значений — личность и роли приходят аргументом, I/O нет, — оно выражается чистой функцией ядра в отдельном модуле со своим типом отказа, а не повторяется в каждом срезе и не уезжает в диспетчер. Там же живёт правило видимости чтения: право команды и видимость чтения — одна система, а не две. Модуль политики ставится в порядке компиляции до агрегата, чтобы читать состояние он мог только снимком.
...
Здесь опорная анатомия расходится с транспортом, и расхождение принудительное. ASP.NET Core принимает реализацию сервиса одним классом-наследником сгенерированной базы, и в нём объявлены **все** операции сервиса сразу. Значит `Api.fs` со своим endpoint внутри каждого среза, как в примере с Oxpecker, для gRPC невозможен: разложить один класс по срезам нельзя.

Oxpecker этого не меняет: для gRPC он не даёт ничего своего и отсылает к штатному ASP.NET Core. Выбор Oxpecker в [ADR-033](../../decisions/ADR-033-meetups-functional-vertical-slices.md) относится к HTTP-границе приложения и на gRPC не распространяется.

Граница раскладывается так:

| Что | Где | Сколько |
|---|---|---|
| класс-наследник `<Service>Base`, регистрация, `MapGrpcService` | `Transport/` приложения | один на сервис |
| интерцептор каркаса лога, health, reflection | `Transport/`, `Observability/`, composition root | один на сервис |
| `Deps`, `Workflow`, отображение error DU в `StatusCode` | `Api` и `Workflow` своего среза | по одному на срез |

- Класс сервиса — **диспетчер**. Тело метода собирает зависимости своего среза, вызывает его workflow и применяет его же отображение результата. Ветвление по содержимому запроса, проверка инвариантов и вызов инфраструктуры в этом классе — дефект границы, а не короткий путь.
- Отображение отказа в `StatusCode` объявляет срез, а не диспетчер. Общий `mapError` на сервис запрещён по той же причине, что и общий `AppError`: он заставляет обрабатывать невозможные для среза случаи и убивает проверку полноты.
- `ServerCallContext` и `Metadata` не проходят глубже диспетчера — ровно как `HttpContext` не проходит глубже `Api`.
- Каркас записи из [logging standard](../observability/logging.md) заполняет интерцептор, а не метод сервиса: граница у сервиса одна, и запись об операции рождается в ней.
- Готовность gRPC-сервис отдаёт по `grpc.health.v1`, а не HTTP-эндпоинтом: plaintext-endpoint без ALPN обслуживает только одну версию протокола, и для gRPC это HTTP/2.

Скелет до первого среза середины не имеет, и это нормально: диспетчер существует, срезов ещё нет. Заводить под них пустые `Slices/<Name>/` заранее нельзя — это тот самый жёсткий шаблон, который ADR-033 отклонил.
...
- Неизвестный вариант enum, несущего состояние, — явный отказ отображения, а не молчаливое значение по умолчанию.
- Неизвестный элемент множества прав отбрасывается, а не отвергается: он ничего не разрешает, поэтому отбрасывание остаётся fail-closed, а отказ по нему превратил бы расширение словаря ролей у владельца схемы в отказ обслуживания у потребителя до согласованного деплоя обоих.
...
Composition root приложения — последний файл перед точкой входа. Он вправе быть и самой точкой входа, и отдельным `Host.fs`: сборка хоста отдельной функцией даёт интеграционному тесту поднять ровно тот же хост, что и запуск, и это единственная причина их разделять.
...
- Generated-типы, `HttpContext`, `ServerCallContext` и строки таблиц не пересекли адаптер.
- Класс gRPC-сервиса остался диспетчером: ни ветвления по запросу, ни вызова инфраструктуры.
