...
Проверка совместимости обязательна: `buf lint` и `buf breaking --against` состояния `origin/develop` идут отдельной джобой CI и рецептом `just`. Она не заменяется ни сборкой потребителей, ни сквозным тестом: потребители, пересобранные из одной и той же сломанной схемы, договорятся между собой, и оба останутся зелёными — так умерла прежняя реализация, где продюсер публиковал Protobuf, а потребитель читал JSON. Принято [RFC-012](../../rfcs/RFC-012-testing-strategy-and-agent-qa.md), вводит [PER-268](https://linear.app/anticnvm/issue/per-268); до её выполнения проверки в репозитории нет, и это состояние, а не разрешение.
