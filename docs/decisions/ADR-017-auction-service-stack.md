...
> **Текущая применимость:** Historical. Свидетельство прежней реализации; стек нового аукциона принимает [PER-145](https://linear.app/anticnvm/issue/per-145). Входными данными для него служат метод сравнения кандидатов и раздел «Что сохраняется», отделивший архитектуру от языка. Конкретные версии, Akka.NET Classic и довод «Event Sourcing из коробки» не переносятся: фреймворк дал персистентность, но не дал корректного восстановления ([архив](../archive/services/auction-domain-and-lessons.md)).
