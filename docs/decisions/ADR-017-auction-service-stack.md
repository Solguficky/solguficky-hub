...
> **Текущая применимость:** Superseded. Заменён [ADR-045](ADR-045-auction-scala-pekko-persistence-jdbc.md): новый сервис использует Scala 3, Apache Pekko Typed и Pekko Persistence JDBC поверх PostgreSQL. Входными данными для преемника послужили метод сравнения кандидатов и раздел «Что сохраняется», отделивший архитектуру от языка. Конкретные версии, Akka.NET Classic и довод «Event Sourcing из коробки» не перенесены: фреймворк дал персистентность, но не дал корректного восстановления ([архив](../archive/services/auction-domain-and-lessons.md)). Замена частичная: разделы про gRPC-фреймворк, NATS-клиент и структуру пакетов преемника не получили — эти вопросы открыты и решаются реализацией первого среза, а ответы этого ADR к Scala не применяются. Тестовый стек Scala выбран [PER-274](https://linear.app/anticnvm/issue/per-274) и записан в [testing-strategy.md](../standards/testing/testing-strategy.md#стек-scala).
