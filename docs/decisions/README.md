...
| [ADR-026](ADR-026-identity-mvp-model-and-access.md) | Модель Identity, контроль состава и проверка доступа в MVP | Active, limited scope | Строки PostgreSQL и журнал доступа, три состояния допуска, премодерация в MVP, gRPC на каждом действии, fail-closed. [ADR-035](ADR-035-temporary-first-admin-bootstrap.md) временно разрешает один bootstrap-id в конфигурации и первичную выдачу роли внутри `ResolveIdentity`; способ входа апдейтов уточнён [ADR-030](ADR-030-telegram-bot.md) |
...
| [ADR-035](ADR-035-temporary-first-admin-bootstrap.md) | Временный bootstrap первого администратора через конфигурацию | Active | Один Telegram user id выдаёт себе первую роль при `ResolveIdentity`; не восстанавливает отозванную роль и удаляется после появления штатного endpoint из PER-30 |
