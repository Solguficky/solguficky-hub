...
| [ADR-036](ADR-036-temporary-first-admin-bootstrap.md) | Временный bootstrap первого администратора из конфигурации | Active, limited scope | `IDENTITY_ADMIN_TELEGRAM_USER_ID` выдаёт первую роль `admin` при `ResolveIdentity`; отозванное не восстанавливается; снимается вместе со служебным endpoint из [PER-30](https://linear.app/anticnvm/issue/per-30). Временно исключает запрет hardcoded-списка и «`ResolveIdentity` роли не выдаёт» из ADR-026 |
