...
| [ADR-024](ADR-024-meetups-state-storage-with-domain-event-log.md) | Внутреннее устройство Meetups: строки состояния плюс журнал доменных событий | Active, limited scope | 1b, чтение из тех же таблиц, версия строки и опрос для отложенной публикации остаются в силе. Роль журнала как outbox заменена [ADR-035](ADR-035-separate-meetups-journal-and-outbox.md); утверждение про diff поправлено ADR-031 |
...
| [ADR-031](ADR-031-meetups-domain-vocabulary-and-event-form.md) | Словарь домена Meetups для среза и форма доменного события | Active, limited scope | Словарь и снимок в силе. Утверждения о журнале как outbox и `position` как курсоре заменены [ADR-035](ADR-035-separate-meetups-journal-and-outbox.md) |
...
| [ADR-035](ADR-035-separate-meetups-journal-and-outbox.md) | Журнал и transactional outbox Meetups разделены | Active | `meetup_events` хранит историю, полный `meetup_outbox` владеет pending- и dispatch-состоянием; high-water cursor по identity запрещён |
