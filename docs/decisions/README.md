...
| [ADR-010](ADR-010-hybrid-auction-clients.md) | Клиенты гибридного аукциона | Needs review | Ни один из клиентов написан не был, поэтому не `Historical`. Экран зала перекрыт [ADR-040](ADR-040-auction-screen-sse.md), посредник Real-Time Hub не вводится. Админка как Mini App открыта и спорит с [mini-app.md](../services/mini-app.md), где Mini App отложен. Пометка снимается решением [PER-147](https://linear.app/anticnvm/issue/per-147) |
...
| [ADR-017](ADR-017-auction-service-stack.md) | C#/Akka.NET Auction Service | Superseded | Заменён [ADR-044](ADR-044-auction-scala-pekko-persistence-jdbc.md) в части языка и персистентности: Scala 3, Apache Pekko Typed и Pekko Persistence JDBC поверх PostgreSQL. Метод сравнения и граница сохраняемых данных использованы как вход, версии и Akka.NET Classic не перенесены. Замена частичная: gRPC-фреймворк, NATS-клиент, тестовый стек и структура пакетов преемника не получили и остаются открытыми — их выбирает реализация первого среза, а прежние ответы к Scala не применяются |
...
| [ADR-044](ADR-044-auction-scala-pekko-persistence-jdbc.md) | Стек и хранение событий Auction Service | Active | Scala 3 и Apache Pekko Typed; полный Event Sourcing через Pekko Persistence JDBC в PostgreSQL. Offset read model хранит Pekko Projection JDBC; аукцион после recovery восстанавливает ссылки на persistent entities лотов |
