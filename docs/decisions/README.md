...
У `Active`-решения, которое более поздний ADR не вытеснил, а дополнил, баннер занимает то же место и называется `> **Дополнение:**`. Он ссылается на дополняющий ADR и говорит, что именно в исходном решении не изменилось. Это единственная вторая форма баннера; для неё индекс обязательным не становится, но строка индекса и баннер так же не расходятся.
...
| [ADR-010](ADR-010-hybrid-auction-clients.md) | Клиенты гибридного аукциона | Superseded | Вытеснен по частям, и каждая часть названа: экран зала — [ADR-040](ADR-040-auction-screen-sse.md); клиент участника — [ADR-044](ADR-044-two-telegram-bots-and-shared-auction-screens.md), два Telegram-входа и общий слой экранов. Admin Panel как Mini App ADR-044 не рассматривает: эту часть вытесняет [mini-app.md](../services/mini-app.md), где Mini App отложен до сценария, который не помещается в бота; консоль аукциониста до тех пор не спроектирована |
...
| [ADR-017](ADR-017-auction-service-stack.md) | C#/Akka.NET Auction Service | Historical | Свидетельство прежней реализации; преемник — [PER-145](https://linear.app/anticnvm/issue/per-145), он принимает Scala + Pekko и хранение событий. Входные данные для него — метод сравнения кандидатов и раздел «Что сохраняется», отделивший архитектуру от языка; версии и Akka.NET Classic не переносятся |
