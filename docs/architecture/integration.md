...
Операция устанавливает личность: создаёт профиль при первом обращении и обновляет ник как кэш. После реализации [PER-167](https://linear.app/anticnvm/issue/per-167) и до появления служебного endpoint она также один раз выдаёт первую роль `admin`, если Telegram user id совпал с временным bootstrap-id процесса ([ADR-035](../decisions/ADR-035-temporary-first-admin-bootstrap.md)); дополнительных полей в wire-контракте для этого нет. Статус допуска, whitelist, инвайты и служебные endpoints премодерации в контракт не входят. Отказы передаются статусами gRPC, отдельного error-message нет.
