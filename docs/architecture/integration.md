...
Сам фильтр видимости остаётся срезом PER-59. До него viewer уже обязателен и проходит общий путь, но текущий reader временно возвращает все сохранённые строки; этот промежуточный API нельзя считать защитой от чтения скрытой сходки. Контрактное правило ниже не меняется: PER-59 реализует его внутри reader, не в Telegram Bot и не в отдельных RPC.
