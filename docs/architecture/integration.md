...
Операция устанавливает личность: создаёт профиль при первом обращении и обновляет ник как кэш. До появления служебного endpoint совпадение с `IDENTITY_ADMIN_TELEGRAM_USER_ID` атомарно выдаёт первую роль `admin` ([ADR-036](../decisions/ADR-036-temporary-first-admin-bootstrap.md)); в коде этой ветки ещё нет. Статус допуска, whitelist, инвайты и служебные endpoints премодерации в этот контракт не входят — полей под них нет. Отказы передаются статусами gRPC, отдельного error-message нет.
