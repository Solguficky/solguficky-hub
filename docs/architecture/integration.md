...
Операция устанавливает личность: создаёт профиль при первом обращении и обновляет ник как кэш. Роли не выдаёт ([ADR-036](../decisions/ADR-036-first-admin-via-service-endpoint.md)). Статус допуска, whitelist, инвайты и служебные endpoints премодерации в этот контракт не входят — полей под них нет. Отказы передаются статусами gRPC, отдельного error-message нет.
