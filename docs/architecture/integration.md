...
Форма публикуемого наружу события среза не касается: без уведомлений событий в шину не уходит. ADR-031 фиксирует доменное событие и строку журнала, а [ADR-035](../decisions/ADR-035-separate-meetups-journal-and-outbox.md) — отдельный transport-neutral outbox с полным event record. Конверт публикации, subject'ы и wire-формат остаются открытыми и решаются вместе со словарём уведомлений. Метод перечисления состояния для реплики Notifications в этот контракт не входит.
