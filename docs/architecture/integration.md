...
Identity публикует события о регистрации и смене статуса допуска: их потребляет Notifications, который ведёт по ним собственную реплику ([ADR-028](../decisions/ADR-028-notifications-subscriptions-replica-and-delivery-boundary.md)). Новый Telegram Gateway потребляет адресные факты уведомлений **собственным** durable consumer: резолвинг получателя, текст, retry и журнал попыток принадлежат каналу ([ADR-030](../decisions/ADR-030-telegram-gateway.md)). Имя subject и конфигурация consumer — contract design.

Gateway устанавливает Telegram identity после проверки входа: secret token вебхука в среде с HTTPS, владение bot token при long polling. Identity разрешает Telegram user id во внутренний id, статус доступа и глобальные роли, а Meetups принимает доменные authorization-решения. Authentication material через Identity не проходит.
