...
Продуктовое ядро сходок не реализовано: Meetups, Telegram Bot и Notifications существуют как принятые решения и контракты, но не как код. Identity разрешает Telegram-личность во внутренний идентификатор. Репозиторий содержит контракты, инфраструктурный задел и инструменты.
...
| Meetups | Граница, техническая модель, стек, внутренние application slices и словарь домена среза Accepted: ADR-024, ADR-025, ADR-031, ADR-033; контракты Open | Владелец продуктовых данных сходок |
