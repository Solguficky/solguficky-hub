...
Например, Meetups относится к MVP, но язык реализации остаётся Open. Scala/Pekko-аукцион относится к Future: стратегическое направление принято, дизайн ещё не начат.
...
Логическая зависимость первого вертикального среза:
...
Это не roadmap и не зафиксированный transport. Срез должен проверить реальные границы сервисов, authentication/authorization, persistence, failure semantics и локальную оркестрацию. Scope и порядок исполнения утверждаются владельцем и фиксируются в Linear.
