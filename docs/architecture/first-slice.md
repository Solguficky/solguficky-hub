...
| Солегуфик | обычный пользователь, круг `member` | внутреннего профиля ещё нет, бота не открывал; его ник заранее внесён администратором в whitelist ([ADR-043](../decisions/ADR-043-identity-roles-and-community-circles.md)) |
...
Одно действие на весь срез. Оно же создаёт профиль при первом обращении, обновляет ник как кэш и гасит совпавшую с ником запись whitelist, выдавая `member` и `public`.
