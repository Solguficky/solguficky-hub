...
- три сервиса разделены реально, а не номинально: Gateway не содержит доменных правил;
- решение об авторизации принимает Meetups, а не Gateway; Gateway передаёт установленную личность и общие роли, но не готовое разрешение;
...
- черновик переживает рестарт сервиса, потому что хранится в Meetups, а не в диалоговом состоянии Gateway;
...
Черновик заводится в Meetups в момент входа в форму, а не собирается целиком в Gateway: это следует из зафиксированного в кадре A-06 правила «неопубликованная сходка хранится в Meetups той же сущностью». Диалоговое состояние Gateway хранит только номер текущего шага. Такое разделение отвечает на открытый вопрос кадра A-03 про незаконченный ввод после рестарта и подлежит подтверждению в ADR Gateway.
...
Внутренний идентификатор, Telegram user id, ник как обновляемый кэш, признак глобальной роли администратора.
...
|---|---|---|
| E-01 | у человека нет прав на административное действие | отказ приходит от Meetups, а не от проверки в Gateway |
...
Отдельно про частичный отказ на записи: если сходка создана, а ответ до Gateway не дошёл, повтор шага не должен завести вторую сходку. Из этого следует требование идемпотентности команды создания, а не только подавления повторов в Gateway. Где живёт ключ идемпотентности — открытый вопрос кадра E-09, решается в ADR Gateway.

Недоступность Identity не деградирует до пропуска проверки: если личность не установлена, операция не выполняется. Вариант «Meetups жив, Identity мёртв, показываем только чтение» остаётся открытым вопросом кадра E-05 и в срезе не реализуется.
...
- премодерация доступа и любая автоматическая проверка членства;
...
- **Конкурентная правка не проверяется.** Редактирования в срезе нет, поэтому сценарий S3 из RFC-004 остаётся без эмпирики, и решение 3 принимается на основании рассуждения, а не наблюдения.
- **Шина не проверяется.** Без уведомлений наружу событий не публикуется, поэтому срез может вообще не задействовать NATS. Сценарий S5 и работа outbox остаются непроверенными до PER-6.
...
- backend runtime и языки Meetups и Identity;
- модель хранения Meetups — решение 1 [RFC-004](../rfcs/RFC-004-meetups-domain-events-persistence.md);
- нужна ли синхронная проверка личности на каждом действии и что происходит при недоступности Identity;
- где живёт диалоговое состояние Gateway и ключ идемпотентности;
- необходимость межсервисных Protobuf-контрактов на этом объёме.

## Связанные документы

- [Архитектурный обзор](overview.md)
- [Meetups](../services/meetups.md), [Identity](../services/identity.md), [Telegram Gateway](../services/telegram-gateway.md)
