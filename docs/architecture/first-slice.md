...
Полная модель Identity из [ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md) шире: у профиля есть роли, отметка блокировки, журнал изменений и whitelist ников. Одноразовые приглашения отложены ([ADR-038](../decisions/ADR-038-identity-hub-access-retention.md)). Целевая модель с тех пор изменилась: статус допуска заменён набором ролей, а блокировка — отдельным полем профиля ([ADR-043](../decisions/ADR-043-identity-roles-and-community-circles.md)). Схема и перенос существующих профилей реализованы в [PER-252](https://linear.app/anticnvm/issue/per-252), но поведение премодерации и расширение контракта остаются отдельными срезами. Премодерация в этот срез не входит: `ResolveIdentity` не выдаёт роли и не проверяет блокировку, а новый профиль создаётся без активных ролей и с `blocked = false`.
