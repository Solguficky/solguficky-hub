...
Ниже перечислено то, что пересекает границу в срезе. Для Identity имя и transport уже зафиксированы контрактом; для Meetups имена условны, а `contracts/proto/` не трогается до утверждения словаря событий в RFC-004.
...
|---|---|---|
| `IdentityService.ResolveIdentity` | Telegram user id, ник если он есть | внутренний идентификатор, набор общих ролей |

Одно действие на весь срез. Оно же создаёт профиль при первом обращении и обновляет ник как кэш. Схема — `contracts/proto/grpc/identity/v1/identity.proto`.
...
- transport операций Meetups: gRPC, NATS или прямой вызов;
- где живёт диалоговое состояние Gateway и ключ идемпотентности;
- межсервисные Protobuf-контракты Meetups; Identity уже описан `ResolveIdentity`.

Закрыто после составления среза: язык Identity — Go ([ADR-027](../decisions/ADR-027-identity-go-stack.md)); проверка личности синхронна на каждом действии, а при недоступности Identity операция завершается fail-closed ([ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md)).
