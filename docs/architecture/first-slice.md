...
Полная модель Identity из [ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md) шире: у профиля есть статус допуска к хабу, а к нему журнал изменений и whitelist ников. Одноразовые приглашения отложены ([ADR-038](../decisions/ADR-038-identity-hub-access-retention.md)). Целевая модель с тех пор изменилась: статус допуска заменён набором ролей, а блокировка — отдельным полем профиля ([ADR-043](../decisions/ADR-043-identity-roles-and-community-circles.md)). Реализованный срез этого ещё не отражает — схема и миграции идут отдельной задачей, — поэтому описанное ниже поведение читается как состояние кода, а не как целевая модель. Премодерация в срез не входит: пути в `allowed` здесь не реализуются, а `ResolveIdentity` статус не проверяет и не возвращает. Профиль при этом создаётся в `pending`, как требует ADR, чтобы допуск не пришлось добавлять миграцией поверх уже работающего продукта.
