...
Полная модель Identity из [ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md) шире: у профиля есть роли, отметка блокировки, журнал изменений и whitelist ников. Одноразовые приглашения отложены ([ADR-038](../decisions/ADR-038-identity-hub-access-retention.md)). Целевая модель с тех пор изменилась: статус допуска заменён набором ролей, а блокировка — отдельным полем профиля ([ADR-043](../decisions/ADR-043-identity-roles-and-community-circles.md)). Схема и перенос существующих профилей реализованы в [PER-252](https://linear.app/anticnvm/issue/per-252), поведение выдачи, отзыва и блокировки — в [PER-191](https://linear.app/anticnvm/issue/per-191), расширение контракта — в [PER-254](https://linear.app/anticnvm/issue/per-254). Проверку роли `member` на границе бота этот срез по-прежнему не доказывает: её закрывает [PER-194](https://linear.app/anticnvm/issue/per-194). `ResolveIdentity` роли не выдаёт и блокировку не проверяет, а новый профиль создаётся без активных ролей и с `blocked = false`.
...
- **премодерация доступа**: whitelist ников, ручной допуск и закрытие доступа. Она входит в MVP по [ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md) и [ADR-038](../decisions/ADR-038-identity-hub-access-retention.md), но не в срез: срез проверяет границы трёх сервисов, и допуск состава к этой проверке ничего не добавляет. Профиль создаётся без активных ролей и с `blocked = false`; выдача роли `member` в срезе не выполняется. Одноразовые приглашения отложены. В целевой модели пути премодерации выдают роли, а не переводят статус ([ADR-043](../decisions/ADR-043-identity-roles-and-community-circles.md));
