...
Черновик заводится в Meetups в момент входа в форму, а не собирается целиком в Gateway: это следует из зафиксированного в кадре A-06 правила «неопубликованная сходка хранится в Meetups той же сущностью». Диалоговое состояние Gateway хранит только курсор шага и ссылку на черновик; атрибуты сходки в курсор не копируются. Такое разделение отвечает на вопрос кадра A-03 про незаконченный ввод после рестарта и закреплено в [ADR-030](../decisions/ADR-030-telegram-gateway.md).
...
Отдельно про частичный отказ на записи: если сходка создана, а ответ до Gateway не дошёл, повтор шага не должен завести вторую сходку. Из этого следует требование идемпотентности команды создания, а не только подавления повторов в Gateway. Ключ выдаёт и сохраняет Gateway до первого вызова; Meetups атомарно связывает его с результатом команды ([ADR-030](../decisions/ADR-030-telegram-gateway.md)).
...
Передаётся в контрактные задачи, а не в задачи реализации среза:

- transport каждой операции: gRPC, NATS или прямой вызов;
- необходимость межсервисных Protobuf-контрактов на этом объёме.

Закрыто после составления среза: язык Identity — Go ([ADR-027](../decisions/ADR-027-identity-go-stack.md)); проверка личности синхронна на каждом действии, а при недоступности Identity операция завершается fail-closed ([ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md)); диалоговый курсор и ключ создания живут по [ADR-030](../decisions/ADR-030-telegram-gateway.md).
...
- [ADR-022](../decisions/ADR-022-meetup-state-axes-and-visibility.md), [ADR-023](../decisions/ADR-023-meetup-public-number.md), [ADR-030](../decisions/ADR-030-telegram-gateway.md)
