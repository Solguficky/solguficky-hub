...
- **Шина не проверяется.** Без уведомлений наружу событий не публикуется, поэтому срез может вообще не задействовать NATS. Сценарий S5 и работа отдельного transactional outbox остаются непроверенными до блока [PER-70](https://linear.app/anticnvm/issue/per-70).
