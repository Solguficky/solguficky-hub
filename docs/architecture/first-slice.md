...
|---|---|---|
| Администратор | глобальная роль администратора, он же автор создаваемой сходки | при первом `/start` роль один раз выдана по временному bootstrap-id из конфигурации ([ADR-035](../decisions/ADR-035-temporary-first-admin-bootstrap.md)) |
...
Внутренний идентификатор, Telegram user id, ник как обновляемый кэш, служебные метки времени профиля, глобальные роли отдельными записями с временем и субъектом выдачи. До появления служебного endpoint один Telegram user id приходит из конфигурации и получает первую роль при `ResolveIdentity`; повторный вызов и отзыв роли не превращают конфигурацию в постоянный источник прав ([ADR-035](../decisions/ADR-035-temporary-first-admin-bootstrap.md)).
...
Закрыто после составления среза: язык Identity — Go ([ADR-027](../decisions/ADR-027-identity-go-stack.md)); проверка личности синхронна на каждом действии, а при недоступности Identity операция завершается fail-closed ([ADR-026](../decisions/ADR-026-identity-mvp-model-and-access.md)); первая роль администратора временно выдаётся по bootstrap-id из конфигурации ([ADR-035](../decisions/ADR-035-temporary-first-admin-bootstrap.md)); состояние экрана и ключ создания живут в самом сообщении, собственного хранилища у бота нет ([ADR-030](../decisions/ADR-030-telegram-bot.md)); продуктовая карточка по умолчанию рисуется `sendRichMessage`, плоский текст остаётся за тоглом процесса ([ADR-034](../decisions/ADR-034-telegram-bot-rich-presentation.md)); шесть операций бот → Meetups — синхронный gRPC ([integration.md](integration.md)).
