...
Профиль `notifications` устроен так же, но зависимость от базы у него жёстче: в его базе лежат не только доменные таблицы, но и membership силоса Orleans, поэтому без строки подключения сервис не просто не слушает — он не поднимает силос вовсе. Миграции применяет тот же DbUp, и он же заводит таблицы Orleans. Шина для него обязательна так же, как база: без `NOTIFICATIONS_NATS_URL` процесс завершается с кодом 1, а без своих durable — останавливается после старта. Так же обязателен пояс сообщества `NOTIFICATIONS_COMMUNITY_TIME_ZONE`: в нём расписание реплики становится моментом начала, от которого считается напоминание, и AppHost отдаёт его той же константой, что Meetups и боту. Порты силоса штатные и берутся из конфигурации: два профиля с Notifications одновременно на одной машине за них подерутся. Право на ручную рассылку Notifications спрашивает у владельцев ресурса: в `depends` стоят `identity` и `meetups`, и bind отдаёт их proxy endpoints в `NOTIFICATIONS_IDENTITY_GRPC_URL` и `NOTIFICATIONS_MEETUPS_GRPC_URL` и ждёт их готовности. Профиль `notifications` соседей не поднимает, bind там молчит, и обе рассылки отвечают `UNAVAILABLE`; живым прогоном связь Notifications → Meetups и Notifications → Identity пока не подтверждена.

Узел `nats` поднимается образом `nats:2.10-alpine` с включённым JetStream и защищён паролем из параметра `nats-password`. Две вещи ломают ожидания и стоят отдельной строки. Порт клиента назначает Aspire, а не 4222: `tools/nats-tester` по умолчанию идёт в `nats://localhost:4222`, поэтому адрес и учётные данные берутся из дашборда и передаются флагом `--nats-url`. JetStream держит store на томе данных дерева (правило ниже), поэтому стримы, сообщения и позиции durable переживают перезапуск AppHost. Streams и durable consumers создаёт сам AppHost, когда узел готов: состав — в [каталоге интеграций](../architecture/integration.md#jetstream), лог узла `nats` пишет `JetStream topology applied` с перечнем имён. Применение идемпотентно, но правку, которую JetStream на живом объекте не принимает, — storage стрима, переход retention в `workqueue` или из него, deliver policy durable (проверено на nats-server 2.10.29; `limits` ↔ `interest` сервер принимает) — сервер отвергает, и лог узла пишет `JetStream topology was not applied`. Узел при этом остаётся `Healthy`, но Notifications его ждёт, и ожидание падает вместе с применением: в профиле с Notifications сломанная топология видна по не стартовавшему сервису, в `infra` — только по логу узла. Лечится удалением тома `nats-data` своего дерева, а с ним и всех сообщений шины; тома других деревьев это не трогает. `just contour-test` из того же дерева пишет в тот же store, что и `aspire run` оттуда же.
...
Продюсеры и потребители шины называют простой NATS по-разному. Identity и Meetups пишут `outbox dispatch` с `dependency_unavailable` на каждом тике релея, пока шина лежит. Потребители — реплика Notifications и канал доставки бота — пишут о соединении, а не о сообщениях: при разрыве чтение durable не падает, а ждёт, и без этой записи простой потребителя неотличим от пустой шины.
...
26. Опубликованная сходка переживает `aspire resource <имя> restart` для Identity и Meetups, а затем `aspire stop` и новый `aspire start` на том же томе: состояние и версия прежние, строка одна, в списке солегуфика она один раз. Брошенная на вопросе о дате форма после рестарта бота отвечает «вопрос устарел», как и задумано [ADR-030](../decisions/ADR-030-telegram-bot.md). Сам черновик в Meetups цел, но на этом прогоне был недостижим из интерфейса: «Управление сходками» предлагало только создать новую. [PER-353](https://linear.app/anticnvm/issue/per-353) добавила раздел «Скрытые сходки», через который черновик открывается обычной карточкой; живым прогоном после рестарта этот путь ещё не подтверждён.
...
Прогон PER-354 от 2026-09-26 — полный `hub` с тем же локальным ботом. Deep link правильной формы на несуществующую сходку (`/start m_` и 22 символа `A`) бот отверг записью `meetup card rejected` с `error_category` = `invariant`, и в Metrics dashboard счётчик `solguficky.failures` появился у ресурса `telegram-bot` со значением 1 и атрибутами `service` и `error_category`. Ресурса `unknown_service:node` в списке нет. До этой правки счётчик бота не экспортировался ни от какого ресурса: он создавался при импорте модуля, раньше чем `startMetrics` ставил провайдер, а у metrics API в JS нет прокси-провайдера, и счётчик оставался no-op. Payload, не прошедший разбор (`/start m_bad`), отказом не считается — бот отвечает приветствием, и счётчик не растёт. Notifications на первом старте упал на `Authentication Timeout` от NATS; `aspire resource notifications start` поднял его без других действий.
...
Прогон PER-7 от 2026-09-26 — полный `hub` на томе своего дерева, с тем же локальным ботом; клиент Telegram вёл владелец. Среда та же, что у PER-228.

35. `just aspire-smoke hub` дважды подряд с нуля дал одинаковый результат. Все 15 ресурсов дошли до конечного состояния: узлы кодогенерации, сборки и установки — `Finished` с кодом 0, остальные — `Running` и `Healthy`. Три сервиса ответили на пробу `SERVING` и на доменный вызов: `ResolveIdentity` завёл профиль, `GetMeetup` отсутствующей сходки вернул `NotFound`, `GetGlobalNotificationPreferences` ответил. Лог `nats` содержит `JetStream topology applied`. Первый `aspire start` на холодном дереве упал на встроенном 120-секундном ожидании CLI: сборка четырёх компонентов идёт дольше. Скрипт поэтому поднимает `ASPIRE_CLI_START_TIMEOUT` до 600.
36. Упавшая сборка различима, но зависимые узлы сами не оживают. `identity-build` упал с `fatal error: runtime: cannot allocate memory` при 774 МБ свободной памяти, `identity` остался в `FailedToStart`, бот — в `Waiting`. После `aspire resource identity-build start` сборка прошла, но `identity` пришлось запускать отдельно, командой `aspire resource identity start`; бот поднялся за ним сам. Smoke называет упавший узел, печатает строку ошибки из его лога и не ждёт до тайм-аута узлы, которые стоят за ним.
37. Одно действие видно одним `request_id` во всех четырёх компонентах. Публикация сходки владельцем через бота дала записи `telegram-bot` (нажатие кнопки), Identity (`ResolveIdentity`, `ResolveTelegramUserId`), Meetups (`PublishMeetup`, `outbox dispatch`) и Notifications (`events.meetups.meetup_published` в реплике), а затем запись бота `notification delivered`. Фильтр по полю `request_id` находит бота, Identity и Meetups. Notifications пишет поля JSON-строкой в теле, поэтому его запись находит только поиск по тексту ([PER-363](https://linear.app/anticnvm/issue/per-363)).
38. После `aspire resource <имя> restart` для `identity`, `meetups`, `notifications` и `telegram-bot` проверка `sh tools/apphost/smoke.sh --attach` зелёная. Бот снова пишет `long polling started`, `409 Conflict` в логе нет.
39. Остановленный PostgreSQL снаружи не отличить от общего тайм-аута (до PER-361; новое поведение — пункты 42–44). Сервисы остаются `Healthy`, проба отвечает `SERVING`, а доменный вызов через 10 секунд возвращает клиенту `DeadlineExceeded`. Причину называют только записи самих сервисов: `dependency_unavailable` у Identity (код `Internal`, уже после отмены клиентом) и у Meetups (код `Unknown`, через 15 секунд). У Notifications записи границы нет ([PER-361](https://linear.app/anticnvm/issue/per-361)). После `aspire resource postgres start` все три сервиса отвечают без рестарта.
40. При остановленном NATS запись продолжается через outbox: сходка публикуется. Identity и Meetups пишут `outbox dispatch` с `dependency_unavailable` и текстом ошибки NATS, а потребители шины — Notifications и бот — не пишут ничего ([PER-362](https://linear.app/anticnvm/issue/per-362)). После `aspire resource nats start` лог `nats` снова пишет `JetStream topology applied`, а накопленные за простой события доходят до реплики Notifications в первые пять секунд.
41. Ручная правка БД не понадобилась ни на одном шаге: роль `admin` владельцу выдана служебным `GrantAdminRole`, как в PER-174.

Разбор и заведённые по расхождениям задачи — в комментарии к [PER-7](https://linear.app/anticnvm/issue/per-7).

Прогон PER-361 от 2026-09-26 — `just aspire-smoke --keep hub --skip-services telegram-bot` на томе своего дерева, затем `aspire resource postgres stop` и `start` на запущенном AppHost. Вызовы — `grpcurl` с `-max-time 3`, то есть с дедлайном бота. Поведение пункта 39 изменено этой задачей ([ADR-054](../decisions/ADR-054-storage-unavailability-visible-outside.md)).

42. Smoke зелёный: проба AppHost и `smoke.sh` спрашивают готовность по имени сервиса, и все три сервиса отвечают `SERVING` на живой базе.
43. При остановленном PostgreSQL доменный вызов каждого сервиса получает `Unavailable` с текстом `storage unavailable` раньше трёх секунд. Записи границы под одним `request_id` несут `grpc_code: Unavailable` и `error_category: dependency_unavailable` у всех трёх, `duration_us` — около 2,0 секунды. Ресурсы на дашборде — `Running` и `Unhealthy`, liveness под пустым именем отвечает `SERVING`, готовность Meetups и Notifications — `NOT_SERVING`. Готовность Identity с клиентским пределом три секунды ответила `DeadlineExceeded`: `grpcurl` тратит часть предела на reflection, а ping базы занимает свои две секунды. С пределом десять секунд она отвечает `NOT_SERVING`.
44. После `aspire resource postgres start` все три сервиса отвечают на первый же доменный вызов и возвращаются в `Healthy` без рестарта процессов.

Более ранние прогоны, которые прогоны PER-228, PER-208, PER-174, PER-5, PER-290, PER-340 и PER-7 не повторяли и не отменяют:

45. Профиль `identity` завершает `identity-proto` и `identity-build` с кодом 0 и доводит Identity до `Healthy`; NATS в этом профиле не поднимается. Identity запущен собранным бинарником из `apps/identity/bin`, получает `IDENTITY_DATABASE_URL` с `sslmode=disable` и слушает назначенный Aspire порт, а после `aspire stop` процесса `identity.exe` в системе не остаётся.
46. Профиль `meetups` после PER-58 поднимает здоровые PostgreSQL, `meetups-db` и Meetups. Полный интеграционный набор с Docker/Testcontainers проходит 53 теста без пропусков.
47. Профиль `notifications` после PER-212 поднимает здоровые PostgreSQL, `notifications-db` и Notifications: в логах видно применение миграций DbUp до подъёма силоса, затем `Orleans Silo started.`, а проба отвечает `SERVING` и через proxy endpoint, и напрямую.
...
Живой gate требует отдельных запусков профилей `infra`, `identity`, `meetups` и `notifications`, среза `hub` без Telegram Bot и его повтора на том же томе, среза по одному сервису, полного `hub` с отдельным локальным ботом и `/start` к нему, а после появления тестового токена — и полного `hub` с `--telegram-environment test`. Токен бота сообщества способом проверки не является ни на одном шаге. Порядок в каждом запуске один, и `just aspire-smoke <профиль>` проходит его механически: дождаться конечного состояния каждого ресурса, сверить граф и health через `aspire describe`, вызвать доменную операцию каждого gRPC-сервиса через найденные в Aspire proxy endpoint и после запуска штатно остановить AppHost. Баннер топологии и логи по-прежнему проверяются глазами. Не используй фиксированный порт: endpoint назначает Aspire. Рестарт компонента и потеря зависимости проверяются на запущенном AppHost (`--keep`): `aspire resource <имя> restart` или `stop`, затем `sh tools/apphost/smoke.sh --attach` и записи сервисов по `request_id`, который печатает скрипт.
