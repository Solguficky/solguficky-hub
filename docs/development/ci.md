...
Джоба `community-site-api` прогоняет typecheck, линт и тесты функций сайта сообщества на изменение `apps/community-site-api/**`, `justfile` и самого workflow. Сборки у компонента нет: функцию бандлит Netlify CLI при деплое, поэтому гейтом остаются проверка типов и тесты доменной логики ревизий — того единственного места, где ошибка стоит потерянных заметок.
