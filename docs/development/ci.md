...
Джоба `contracts` собирает модуль `contracts/proto` целиком (`buf build`) на изменение `contracts/proto/**`, `justfile` и самого workflow. Она не дублирует джобы потребителей, а закрывает их слепое пятно: кодогенерация Identity и Telegram Bot сужает вход фильтром `paths`, а `Meetups.Contracts` перечисляет файлы поимённо, поэтому схема домена, у которого потребителя ещё нет, не читается ни одной из них и ломается молча при зелёном CI. Так сейчас устроен `notifications/v1`. Стилевых правил джоба не проверяет: `buf lint` красный на `meetups_service.proto`, где общий ответ пяти команд — принятое решение, а не дефект, и подключение `buf lint` вместе с breaking check остаётся за [PER-268](https://linear.app/anticnvm/issue/per-268). Версию `buf` джоба читает из `BUF_VERSION` в `justfile`, как `identity` и `telegram-bot`. Тот же гейт локально — `just contracts-build`, входящий в `just verify`.
...
- `buf lint` и compatibility check Protobuf ещё не внедрены: модуль сегодня только компилируется джобой `contracts`;
