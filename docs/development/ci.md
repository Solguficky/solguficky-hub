...
Джоба `community-site-api` прогоняет typecheck, линт и тесты сайта сообщества на изменение `apps/community-site-api/**`, `justfile` и самого workflow. Сборки у компонента нет: функцию бандлит Netlify CLI при деплое, поэтому гейт держат проверка типов и четыре слоя тестов — логика документа, граница функции с подменённым хранилищем, страница в jsdom и сценарии в настоящем браузере.
