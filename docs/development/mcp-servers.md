...
| Разобрать Loki при красном живом гейте | живой гейт и остановка | Grafana UI, файлы логов, Aspire | возможно, Grafana MCP |
...
| Посмотреть схему и данные тестовой PostgreSQL при падении интеграционного теста | `just identity-test-integration` | миграции, тесты и SQL-клиент | отдельный MCP не окупается |
...
> **Дополнение 2026-09-20.** Третье основание отказа перестало действовать в той форме, в какой записано. «Полноценная сессия человека» была свойством конфигурации, а не протокола: [Telegram документирует выделенную тестовую среду](https://core.telegram.org/bots/features) с отдельными дата-центрами и отдельным BotFather, а [описание авторизации](https://core.telegram.org/api/auth) — синтетические номера `99966XYYYY` с кодом из цифры дата-центра, повторённой пять раз. Аккаунт в такой среде не принадлежит человеку и не содержит ни контактов, ни переписки; Telegram сам предупреждает не хранить там ничего важного. Два первых основания в силе, и рекомендация раздела не меняется — Telegram MCP общего назначения не подключается. Меняется судьба вынесенного отсюда сценария: обещанный «отдельный тестовый контур» принят [ADR-046](../decisions/ADR-046-telegram-test-contour.md).
