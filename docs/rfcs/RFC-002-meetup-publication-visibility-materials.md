...
1. Число осей состояния и их набор — решение 1. Блокирует схему Meetups.
2. Обратима ли публикация и что происходит с подписками при снятии — решение 2.
3. Где живёт черновик и когда проверяются обязательные атрибуты — решение 3.
4. Нужно ли уведомление о новой сходке и кто им управляет — решение 4. При выборе 4b появляется подписка без сходки, чего в модели MVP нет.
5. Нужна ли ограниченная аудитория вообще и чем она задаётся — решение 5.
6. Постеры как атрибут или как материал, и где лежит изображение — решение 8.
7. Кто может добавить неофициальную афишу и кто может её убрать.
8. Остаётся ли отменённая сходка видимой и как долго.

Требует проверки, а не решения:

9. При варианте 5a — может ли бот получить состав участников закрытого чата, не нарушая принцип минимальных прав и Privacy Mode.

## Результирующие артефакты

- **ADR:** вероятно один — оси состояния сходки и правила видимости. Остальное относится к продуктовой модели, а не к архитектуре. Создаётся после решения владельца.
- **Standard:** не нужен.
- **Обновление документов:** [product/overview.md](../product/overview.md) — состояния переходят из открытых вопросов в модель; [services/meetups.md](../services/meetups.md) — правила видимости и владение аудиторией; [design/bot/storyboard.html](../design/bot/README.md) — кадры A-03…A-07 и открытые вопросы про черновик; [product/ideas.md](../product/ideas.md) — строки про постеры и ограниченную аудиторию закрываются или уточняются.
- **Задачи Linear:** после принятия.
- **`contracts/proto/`:** не трогается до принятия решений 1 и 5.
