...
> **Статус:** Draft, документарный слой проверен 2026-09-06; живой прогон клиентов не выполнен
> **Автор:** владелец
> **Дата:** 2026-08-12
...
Это меняет не оформление отдельных кадров, а словарь, которым они собраны: часть экранов и переходов перестаёт быть нужной. RFC фиксирует варианты и их цену. Ни один вариант здесь не принят. Документарный слой Bot API 10.3 и типы grammY 1.46 больше не держат развилку на неизвестности `reply_markup`; решение владельца всё ещё ждёт наблюдаемого поведения на клиентах.
...
| Параметр `reply_markup` у `sendRichMessage` | есть в справочнике метода на 6 сентября 2026; в changelog 10.1/10.2 не упоминался | документарный гейт «клавиатура на карточке» снят; живой отказ API всё ещё возможен |
...
| C. Живой бот в личном чате и тестовой группе | принимает ли сервер вызов, приходит ли callback, как рисуют mobile/desktop, нужен ли Premium читателю | не выполнен |
...
|---|---|---|---|---|---|
| `sendRichMessage` + `reply_markup` (inline) | слой A: параметр объявлен; слой B: поле есть в `other` | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| callback после inline-клавиатуры на rich-сообщении | слой A: `callback_data` у `InlineKeyboardButton` без оговорки для rich | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| `InputRichBlockButtons` с `callback_data` | слой A: 1–8 кнопок в ряду, `callback_data` 1–64 байта; слой B: тип есть | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| Чтение без Premium | слой A: ограничения у читателя нет; Premium упомянут у редактора людей и у custom emoji отправителя | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| Ограничение «AI bots» | слой A: в справочнике метода нет; в [features](https://core.telegram.org/bots/features#rich-messages) rich-сообщения описаны как возможность ботов вообще, не подкласса | слой C: не прогнано | — | — | — |
| `editMessageText` с `rich_message` | слой A: метод правит «text, rich and game messages»; слой B: поле типизировано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| Эфемерная команда (`BotCommand.is_ephemeral`) | слой A: команда видна боту и отправителю, остальным членам чата — нет | — | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| Эфемерный ответ (`ephemeral_message_parameters`) | слой A: окно 15 с от eligible action, либо бот-админ без окна; `message_id = 0`; доставка offline не гарантирована | не применяется | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
...
- Тот же метод принимает `ephemeral_message_parameters`: `{ receiver_user_id, callback_query_id?, replace_callback_query_message? }`.
- Целый rich-документ правится через `editMessageText(..., rich_message)`; отдельного `editRichMessage` нет.
- С 10.3 действие может стоять внутри документа: `RichMessageButton.callback_data` в блоке `buttons` или как `RichTextButton` в тексте. Это снимает жёсткую границу «в блоке только ссылка», сформулированную до 10.3. Живым ботом не подтверждено, как ряд кнопок выглядит на карточке сходки и приходит ли callback с тем же дедлайном `answerCallbackQuery`.
...
- grammY 1.46.0, уже стоящий в `apps/telegram-bot`, оборачивает `sendRichMessage`, `sendRichMessageDraft` и `editEphemeralMessage*`. Raw Bot API для этих вызовов не нужен: третий аргумент `other` несёт `reply_markup` и `ephemeral_message_parameters`.

#### Гипотезы

- Сгиб «показать ещё» около 8 000 символов и потолок 32 768 по-прежнему взяты из анонса, а не из таблицы параметров метода.
- Обычный бот, не помеченный как AI, может отправить rich-сообщение: справочник этого не запрещает, живой отказ сервера не проверялся.
...
- Примет ли `api.telegram.org` `reply_markup` на конкретном токене или ответит 400.
- Придёт ли `callback_query` с inline-клавиатуры и с `RichBlockButtons`.
- Видит ли непремиум-читатель документ целиком на актуальных mobile и desktop.
- Как выглядит и сколько живёт эфемерный ответ, если получатель offline.
- Пригодны ли блоки 10.2/10.3 для карточки сходки на реальных клиентах, а не на бумаге.
...
Пока слой C пуст, карточка сходки остаётся вариантом **1a**: `sendMessage` плюс inline-клавиатура, навигация — уже выбранный вариант B, один хаб-экран. Это не закрывает 1d и не вводит D: это нижняя граница, на которой продукт уже умеет отвечать.

Практическое правило до решения владельца:

1. Продуктовый бот шлёт только `sendMessage` / `editMessageText` без `rich_message`.
2. Если слой C подтвердит `reply_markup` и чтение без Premium, владелец выбирает, открывать ли 1d / 3b / 4b.
3. Если слой C отвергнет клавиатуру, Premium или деградацию на desktop — 1d закрывается, fallback остаётся единственной карточкой, постеры идут в 1b.
...
**Граница действий в блоке.** До 10.3 внутри блока размещалась ссылка, но не кнопка: inline-клавиатура принадлежала сообщению, а не блоку. Bot API 10.3 объявил `InputRichBlockButtons` и `RichTextButton` с тем же `callback_data` на 64 байта. Документарно действие по элементу больше не обязано уезжать на общую клавиатуру. Живым ботом не проверено, встаёт ли ряд кнопок напротив строки списка и пригоден ли он для «убрать связь» из кадра A-14. Пока слой C пуст, расчёт карточки держится на `reply_markup` сообщения.
...
1. Объявлен ли `reply_markup` у `sendRichMessage` — да, в справочнике метода. Примет ли сервер вызов — неизвестно.
2. Поддержка в grammY 1.46.0 — да, типизированный метод, raw API не нужен.

Требуют живого прогона до решения:

3. Требование Premium у читателя.
4. Ограничение «AI bots» как отказ обычного бота.
5. Поведение на актуальных mobile и desktop.
6. Callback с `reply_markup` и с `InputRichBlockButtons`.
7. Эфемерная команда, адресный ответ, жизнь сообщения и поведение offline.
