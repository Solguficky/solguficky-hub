...
> **Статус:** Draft, живой прогон 2026-09-06: личка, группа и desktop; mobile, Premium и видимость шёпота ещё открыты
> **Автор:** владелец
> **Дата:** 2026-08-12
...
| C. Живой бот в личном чате и тестовой группе | принимает ли сервер вызов, приходит ли callback, как рисуют mobile/desktop, нужен ли Premium читателю | личка и группа, API: выполнен 6 сентября 2026, `@solguficky_stage_bot`; desktop по скрину владельца; mobile, Premium и видимость шёпота — нет |
...
| callback после inline-клавиатуры на rich-сообщении | слой A: `callback_data` у `InlineKeyboardButton` без оговорки для rich | слой C: `per20:ping`, ответ принят | слой C: ждёт нажатия в группе | слой C: не прогнано | слой C: кнопка нажимается, query пришёл из лички |
| `InputRichBlockButtons` с `callback_data` | слой A: 1–8 кнопок в ряду; слой B: тип есть | слой C API: `200`; `per20:open` пришёл | слой C API: `200`, ряд ушёл вместе с публичной карточкой | слой C: не прогнано | слой C: кнопка `Открыть` под секцией, query из лички пришёл |
| Чтение без Premium | слой A: ограничения у читателя нет | слой C: не отделено от Premium-статуса владельца | слой C: не прогнано | слой C: не прогнано | слой C: документ читается; Premium у читателя не зафиксирован |
...
| Эфемерный ответ (`ephemeral_message_parameters`) | слой A: `message_id = 0`; offline не гарантирован | не применяется | слой C API: `200`, `message_id = 0`; второй участник тоже получил свой вызов | слой C: не прогнано | слой C: у адресата пометка «Видите только вы»; видит ли его другой человек — ещё нет |
...
- Живой обычный бот `@solguficky_stage_bot`: `sendRichMessage` с inline-клавиатурой вернул `200` в личке и в группе. Ограничение «AI bots» не сработало.
- Тот же метод принимает `ephemeral_message_parameters`. Живой вызов в тестовой группе вернул `200`, `message_id = 0` и ненулевой `ephemeral_message_id`. Видит ли сообщение только адресат — ещё не подтверждено глазами.
...
- Пригодны ли блоки для карточки сходки на mobile; на desktop по скрину владельца — да.
- Видит ли эфемерный ответ другой член группы, в том числе без Premium. У адресата на desktop уже есть пометка «Видите только вы».
...
API-гейт `reply_markup` зелёный в личке и в группе; desktop рисует карточку. Пока нет mobile, Premium и проверки «шёпот виден только адресату», продуктовая карточка остаётся вариантом **1a**: `sendMessage` плюс inline-клавиатура, навигация — вариант B. Это не закрывает 1d и не вводит D.
...
6. Требование Premium у читателя.
7. Вид на mobile.
8. Виден ли эфемерный ответ только адресату; жизнь сообщения и offline.
9. Эфемерная команда через `BotCommand.is_ephemeral` — не прогоняли.
