...
> **Статус:** Draft, слой A/B и API личного чата проверены 2026-09-06; вид на клиентах и группа ещё открыты
> **Автор:** владелец
> **Дата:** 2026-08-12
...
Это меняет не оформление отдельных кадров, а словарь, которым они собраны: часть экранов и переходов перестаёт быть нужной. RFC фиксирует варианты и их цену. Ни один вариант здесь не принят. Документарный слой Bot API 10.3, типы grammY 1.46 и живой вызов в личном чате сняли неизвестность `reply_markup`: сервер принял клавиатуру. Решение владельца всё ещё ждёт вида на клиентах и прогона в группе.
...
| C. Живой бот в личном чате и тестовой группе | принимает ли сервер вызов, приходит ли callback, как рисуют mobile/desktop, нужен ли Premium читателю | личный чат: API и оба callback 6 сентября 2026, обычный бот `@solguficky_stage_bot`; вид на клиентах и группа — нет |
...
|---|---|---|---|---|---|
| `sendRichMessage` + `reply_markup` (inline) | слой A: параметр объявлен; слой B: поле есть в `other` | слой C API: `200`, в ответе есть `rich_message` и `reply_markup` | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| callback после inline-клавиатуры на rich-сообщении | слой A: `callback_data` у `InlineKeyboardButton` без оговорки для rich | слой C: пришёл `callback_query` `per20:ping`, `answerCallbackQuery` = `true` | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
| `InputRichBlockButtons` с `callback_data` | слой A: 1–8 кнопок в ряду, `callback_data` 1–64 байта; слой B: тип есть | слой C API: `200` без `reply_markup` сообщения; пришёл `callback_query` `per20:open`, ответ принят | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
...
| Ограничение «AI bots» | слой A: в справочнике метода нет; в [features](https://core.telegram.org/bots/features#rich-messages) rich-сообщения описаны как возможность ботов вообще, не подкласса | слой C API: обычный бот отправил rich-сообщение, отказ «AI only» не пришёл | — | — | — |
| `editMessageText` с `rich_message` | слой A: метод правит «text, rich and game messages»; слой B: поле типизировано | слой C API: `200`, тот же `message_id`, `rich_message` и `reply_markup` на месте | слой C: не прогнано | слой C: не прогнано | слой C: не прогнано |
...
- Живой обычный бот в личном чате: `sendRichMessage` с inline-клавиатурой вернул `200`, в `Message` есть и `rich_message`, и `reply_markup`. Ограничение «AI bots» на этом токене не сработало.
- Тот же метод принимает `ephemeral_message_parameters`: `{ receiver_user_id, callback_query_id?, replace_callback_query_message? }`. В группу ещё не посылали.
...
- С 10.3 действие может стоять внутри документа: `RichMessageButton.callback_data` в блоке `buttons` или как `RichTextButton` в тексте. Живой `sendRichMessage` блоками вернул `200` без `reply_markup` сообщения; нажатие ряда дало `callback_query` `per20:open`, `answerCallbackQuery` прошёл. Нажатие inline-клавиатуры на том же типе сообщения дало `per20:ping`.
...
API-гейт `reply_markup` в личном чате зелёный. Пока нет наблюдения на клиентах и прогона в группе, продуктовая карточка остаётся вариантом **1a**: `sendMessage` плюс inline-клавиатура, навигация — уже выбранный вариант B. Это не закрывает 1d и не вводит D.
...
1. Объявлен ли `reply_markup` у `sendRichMessage` — да. Сервер принял вызов в личном чате.
...
4. Ограничение «AI bots» — обычный бот отправил rich-сообщение.

Требуют наблюдения на клиентах и прогона в группе:

5. Требование Premium у читателя.
6. Поведение на актуальных mobile и desktop.
