...
> **Статус:** Draft, живой прогон 2026-09-06: личка, группа, desktop, чтение без Premium; mobile и взаимная видимость шёпота ещё открыты
> **Автор:** владелец
> **Дата:** 2026-08-12
...
| C. Живой бот в личном чате и тестовой группе | принимает ли сервер вызов, приходит ли callback, как рисуют mobile/desktop, нужен ли Premium читателю | личка и группа 6 сентября 2026, `@solguficky_stage_bot`; desktop; читатель без Premium видит карточку и жмёт кнопки; mobile и взаимная видимость шёпота — нет |
...
- Видит ли эфемерный ответ другой член группы. У адресата на desktop есть пометка «Видите только вы»; взаимную невидимость ещё не сверили.
...
API-гейт `reply_markup` зелёный в личке и в группе; desktop рисует карточку; Premium читателю не нужен. Пока нет mobile и сверки «чужой шёпот не виден», продуктовая карточка остаётся вариантом **1a**: `sendMessage` плюс inline-клавиатура, навигация — вариант B. Это не закрывает 1d и не вводит D.
...
6. Premium читателю не нужен: непремиум-участник видит документ и жмёт кнопки.

Требуют наблюдения:

7. Вид на mobile.
8. Виден ли эфемерный ответ другому члену группы; жизнь сообщения и offline.
