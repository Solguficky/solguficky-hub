...
Оба хвоста про шёпот в группе, не про публичную карточку. Гейт 1d они не держат.

**Offline-доставка эфемерного ответа.** Речь про `ephemeral_message_parameters`, тот самый ответ с пометкой «Видите только вы». Живой прогон слал шёпот человеку, который в этот момент был в чате, и он его увидел — без Premium тоже. Не проверяли другой случай: адресат офлайн, бот шепнул, человек потом открыл Telegram. Справочник прямо говорит, что такая доставка не гарантирована; то же для правки и удаления шёпота. Неизвестно, придёт ли сообщение позже, как оно выглядит и сколько живёт. Для публичной карточки это не важно. Станет важно, если в группе начнём шептать отказ или настройку и на этом строить сценарий: офлайн-адресат может просто не увидеть ответ.

**Эфемерная команда.** Это другое API, не шёпот-ответ. У пункта меню бота есть флаг `BotCommand.is_ephemeral`; его ставят через `setMyCommands`. Человек жмёт `/команду` в группе — эту команду видят только он и бот, остальным в чате её нет. В меню такая команда помечается отдельной иконкой. `setMyCommands` с этим флагом не вызывали. На карточку и на уже выбранный вариант B это не влияет. Понадобится, если служебные команды в общем чате не должны светиться у всех.
...
Требуют наблюдения, гейт не держат. Разбор — в [«Неизвестные»](#неизвестные--только-слой-c):

8. Offline-доставка шёпота: адресат был офлайн в момент `ephemeral_message_parameters`, потом открыл клиент.
9. Эфемерная команда: `setMyCommands` с `BotCommand.is_ephemeral` — `/команда` в группе видна только отправителю и боту.
