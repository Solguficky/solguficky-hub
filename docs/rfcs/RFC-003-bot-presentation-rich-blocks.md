...
| **1d. `sendRichMessage`** | блоки: заголовок, таблица, сворачиваемые секции, слайдшоу, с 10.3 — кнопки внутри документа | закрывает постеры, настройки и содержимое модулей одним механизмом; правка на месте через `rich_message` | две реализации, если нужен откат на 1a; mobile и сгиб остаются хвостами наблюдения |
...
Опционально `TELEGRAM_BOT_PRESENTATION=plain` собирает ту же карточку через `sendMessage`. Дефолт — `rich`. Это тот же тогл, что в [ADR-034](../decisions/ADR-034-telegram-bot-rich-presentation.md); зонд не является продуктовым ботом.
...
2. Продуктовая карточка из одной экранной модели: в `rich` — `sendRichMessage` блоками и клавиатурой хаба; в `plain` — `sendMessage` с тем же смыслом и обеими кнопками в `reply_markup`.
3. Правка той же карточки на месте: `editMessageText` с `rich_message` или с `text`.
4. В группе — эфемерный ответ тем же методом, что и выбранный режим.
5. Печатает JSON: `presentation`, метод, HTTP-статус, `ok`, `error_code`, `description`, идентификаторы.
