...
| Параметр `rich_message` у `editMessageText` | 10.1, 11 июня 2026 | rich-сообщение, судя по всему, правится на месте |
...
| Поддержка в grammY | гейт для стека бота | чтение исходников или пробный вызов |
...
Проверка 1 требует уточнения формулировки: в changelog Bot API 10.1 и 10.2 параметр `reply_markup` у `sendRichMessage` не упоминается ни в одну сторону, поэтому из истории изменений ответ не выводится. Смотреть нужно текущее описание метода в справочнике либо проверять живым ботом.
