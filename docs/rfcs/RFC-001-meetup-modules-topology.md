...
**Внешняя зависимость этого решения.** Механика карточки принята: дефолт — `sendRichMessage`, плоский текст за тоглом процесса ([ADR-034](../decisions/ADR-034-telegram-bot-rich-presentation.md), [RFC-003](RFC-003-bot-presentation-rich-blocks.md)). Нижняя граница «строка и действия» больше не единственная форма. Что модуль отдаёт shell — решение 3 RFC-003 — по-прежнему открыто и блокирует `.proto`. Правило роста контракта не меняется.
...
11. Чем ограничен вклад модуля в карточку — механика принята ([ADR-034](../decisions/ADR-034-telegram-bot-rich-presentation.md)); форма вклада (3a / 3b / 3c) остаётся решением 3 [RFC-003](RFC-003-bot-presentation-rich-blocks.md) и блокирует `.proto`.
