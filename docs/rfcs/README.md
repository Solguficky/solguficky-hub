...
| [RFC-007](RFC-007-remote-development-and-self-hosting-platform.md) | Удалённая среда разработки и self-hosting Solguficky | In Review; hosting model принят в ADR-034 |

Документы разделены по границе, которую каждый блокирует: RFC-001 — контракт расширения, RFC-002 — продуктовую модель сходки, RFC-003 — форму сообщений бота и ревизию макета, RFC-004 — схему и язык Meetups, RFC-005 — устройство Notifications и границу доставки, RFC-006 — устройство Telegram-края и его состояние, RFC-007 — модель удалённой разработки, self-hosting и восстановления.
