...
- алерты этого RFC и внешний dead-man's switch исполняет облачный бэкенд наблюдаемости, а на хосте работает только Collector: сервисы шлют OTLP в него, бэкенд — Better Stack ([ADR-draft](../decisions/ADR-draft-production-observability-otlp-better-stack.md)). Журнал хоста хранит логи дольше окна бэкенда, его срок задаёт host baseline.
