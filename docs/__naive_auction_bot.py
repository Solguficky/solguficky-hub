...
CREATOR_ID = 1337  # Ваш Telegram ID
bids = {}  # Словарь для хранения ставок
# Пример данных для лотов с учетом рублевой стоимости и минимального шага
auction_lots = {
    1: {
        'title': 'Значок "Собака"',
        ...
        'Автор - Нян',
        'current_bid': None,
        'current_bidder_id': None,  # ID пользователя, который сделал текущую максимальную ставку
        'min_bid_step': 50.0,
        'bids': [],
        'max_bid': {},  # Добавьте это поле для отслеживания максимальных ставок по пользователям
        'last_bid': {},  # Инициализация словаря для последних ставок
    ...
    2: {
        ...
        'image_url': 'https://imgur.com/a/YB0awqD'  # URL изображения
    },
    3: {
        ...
        'image_url': 'https://imgur.com/a/YS75ov9'  # URL изображения
    },
    4: {
        'title': 'Значок "Хрю"',
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/Mifg2Cr'  # URL изображения
    },
    5: {
        'title': 'Значок "Портал"',
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/YKLNNEw'  # URL изображения
    },
    6: {
        'title': 'Значок "Пицца"',
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/tuQc6eK'  # URL изображения
    },
    7: {
        ...
        'image_url': 'https://imgur.com/a/SA71XIm'  # URL изображения
    },
    8: {
        'title': 'Значок "Клоун"',
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/U4WdFYF'  # URL изображения
    },
    9: {
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 500.0,
        'image_url': 'https://imgur.com/a/gavAKem'  # URL изображения
    },
    10: {
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/ShVgfrC'  # URL изображения
    },
    11: {
        ...
        'last_bid': {},  # Инициализация словаря для последних ставок
        'starting_price': 100.0,
        'image_url': 'https://imgur.com/a/qWcYncb'  # URL изображения
    },
    12: {
        ...
        'image_url': 'https://imgur.com/a/G7g3cMM'  # URL изображения
    },
}
...
# Функция стартового сообщения с кнопками
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data['first_message'] = True  # Установка флага для нового пользователя
    await update.message.reply_text(welcome_message, reply_markup=reply_markup)

def create_bid(user_id, amount):
    global next_bid_id
    bid_id = next_bid_id
    next_bid_id += 1  # Увеличиваем глобальный ID для следующей ставки
    # Сохраняем ставку в общем хранилище (например, в словаре)
    bids[bid_id] = {'user_id': user_id, 'amount': amount}
    return bid_id

# Функция для обработки возврата к началу
...
next_bid_id = 1  # Глобальная переменная для ID ставок

async def show_user_bids(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    user_bids = {}

    # Сбор текущих максимальных ставок для каждого лота пользователя
    for lot_id, lot_info in auction_lots.items():
        # Извлечь максимальную ставку пользователя, если она существует
        user_bid = max((bid for bid in lot_info['bids'] if bid['user_id'] == user.id), key=lambda x: x['amount'], default=None)

        if user_bid:
            current_bid = lot_info['current_bid']  # Текущая максимальная ставка лота
            
            # Проверяем, если пользователь сделал ставку на этот лот
            user_bids[lot_id] = {
    ...
    if user_bids:
        ...
        for lot_id, bid_info in user_bids.items():
            current_bid = bid_info['current_bid']  # Текущая максимальная ставка за лот.

            # Определяем, кто сделал текущую максимальную ставку
            current_bid_user = next((bid for bid in auction_lots[lot_id]['bids'] if bid['amount'] == current_bid), None)
            current_bid_user_name = "Неизвестный"
            if current_bid_user:
                current_bid_user_name = (await context.bot.get_chat(current_bid_user['user_id'])).first_name

            # Создаем строку сообщения о текущей максимальной ставке по лоту
...
async def delete_bid(update: Update, context: CallbackContext) -> None:
    ...
    if user.id != CREATOR_ID:
        ...
        await update.message.reply_text('У вас нет прав на удаление ставок. Рапорт уже составлен')  # Добавлено await
        return

    # Проверяем, что ID ставки был передан
    if len(context.args) != 1:
        await update.message.reply_text("Пожалуйста, укажите ID ставки для удаления.")
        context.user_data['awaiting_bid_id'] = True  # Ставим флаг, что ожидаем ввод ID
async def process_delete_bid_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if 'awaiting_bid_id' in context.user_data and context.user_data['awaiting_bid_id']:
        user = update.effective_user
        bid_id_str = update.message.text.strip()  # Получаем текст и убираем лишние пробелы
        ...
        # Если не ожидали ввода ID, можно вызвать стандартное поведение
        await handle_unknown_message(update, context)

# Функция для уведомления перебитых пользователей
async def notify_outbid_users(lot, new_max_bid_amount, context):
    for bid in lot['bids']:
        if bid['amount'] < new_max_bid_amount:
            user_id = bid['user_id']
            # Вместо отправки уведомления всем, отправляем только владельцу этой ставки
            try:
...
# Обработка нажатия кнопки "Показать лоты"
...
async def lot_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Определяем доступные кнопки в зависимости от текущей ставки
    starting_price = lot['starting_price']
    min_bid_step = lot['min_bid_step']  # Определяем минимальный шаг

    buttons = []

    if lot['current_bid'] is None:
        # Если ставок еще нет, добавляем кнопку "Посмотреть описание"
        ...
        new_bid = lot['current_bid'] + min_bid_step  # Рассчитываем новую ставку
        buttons.append([InlineKeyboardButton("Посмотреть описание", callback_data=f'description_{lot_id}')])
    ...
    await query.edit_message_text(
        text=f"Лот: {lot['title']}\n"
             f"Текущая ставка: {lot['current_bid'] if lot['current_bid'] else 'Нет ставок'}\n"
             "Выберите действие:",
        reply_markup=reply_markup
    )

# Функция для возврата к списку лотов
async def go_to_lots(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.callback_query.answer()  # Подтверждаем нажатие на кнопку
    # Снова показываем список лотов
    await show_lots(update, context)

# Функция для обработки начального торга
async def start_bid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Устанавливаем текущую ставку как стартовую
    auction_lots[lot_id]['current_bid'] = starting_price
    auction_lots[lot_id]['bids'].append({'user_id': query.from_user.id, 'amount': starting_price})  # Сохраняем стартовую ставку
...
async def bid_increase(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Автоматическое повышение ставки
    new_bid = lot['current_bid'] + lot['min_bid_step']
    
    # Сохраняем новую ставку и получаем её ID
    bid_id = create_bid(query.from_user.id, new_bid)
    lot['bids'].append({'user_id': query.from_user.id, 'amount': new_bid})  # Сохраняем автоматическую ставку
    
    # Обновляем последнюю ставку пользователя
    if query.from_user.id not in lot['last_bid']:
        lot['last_bid'][query.from_user.id] = new_bid
    else:
        lot['last_bid'][query.from_user.id] = new_bid  # Обновляем последнюю ставку пользователя

    # Проверяем, нужно ли уведомлять о перебитии
    if new_bid > lot['current_bid']:
        lot['current_bid'] = new_bid
        await notify_outbid_users(lot, new_bid, context)
...
async def list_lots(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка, является ли запрашивающий пользователь создателем бота
    if user.id != CREATOR_ID:
        await update.message.reply_text("У вас нет доступа к этой команде.")
        return

    message_lines = []  # Инициализируем список строк для отображения

    # Сбор информации о всех лотах
    for lot_id, lot_info in auction_lots.items():
...
# Функция для обработки индивидуальной ставки
async def set_individual_bid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        text=f"Введите вашу индивидуальную ставку для '{auction_lots[lot_id]['title']}'.\n"
             f"Текущая ставка: {auction_lots[lot_id]['current_bid'] if auction_lots[lot_id]['current_bid'] else 'Нет ставок'} рублей.\n"
...
async def process_bid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if lot['current_bid'] is not None and bid_amount <= lot['current_bid']:
            await update.message.reply_text(f"Ваша ставка должна быть больше текущей ставки ({lot['current_bid']}).")
            return

        # Сохраняем новую ставку
        bid_id = create_bid(user.id, bid_amount)  # Получаем ID ставки
        lot['bids'].append({'user_id': user.id, 'amount': bid_amount})  # Сохраняем ставку

        # Условия для обновления максимальной ставки
        if lot['current_bid'] is None or bid_amount > lot['current_bid']:
            lot['current_bid'] = bid_amount
            await notify_outbid_users(lot, bid_amount, context)  # Уведомляем пользователей о перебитии

        # Отправляем сообщение с ID ставки пользователю
        await update.message.reply_text(
        ...
        await handle_unknown_message(update, context)  # Здесь вызываем функцию обработки неизвестного сообщения

# Функция для обработки описания лота
async def show_description(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка прямого URL изображения
    ...
    try:
        # Отправка изображения и текста описания
        await context.bot.send_photo(
            chat_id=query.message.chat.id,  # Используйте chat.id
            photo=image_url,
            caption=(
    ...
    # Определяем кнопки для повышения ставки и индивидуальной ставки
    buttons = []
    starting_price = lot['starting_price']
    ...
    if lot['current_bid'] is None:
        ...
        # Расчет новой ставки
        new_bid = lot['current_bid'] + min_bid_step
        # Добавление кнопки с информацией о минимальном шаге
        buttons.append([
...
# Функция для обработки команды auction
async def auction_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Проверяем, является ли это callback_query или текстовое сообщение
    if update.callback_query:
        query = update.callback_query
        await query.answer()
        message = query.message
    else:
        # Если это не коллбек, значит это текстовое сообщение
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    message = update.message  # Получаем сообщение
...
def main() -> None:
    """Запустите бота."""
    app = ApplicationBuilder().token("tuttoken").build()
