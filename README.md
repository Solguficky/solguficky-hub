...
Профиль перечисляет узлы, которыми AppHost владеет в запуске, и задаётся данными в `Topology:Profiles`; сейчас определены `infra`, `identity`, `meetups` и `hub`. Живая проверка готовности после переработки графа ещё не повторена. Команды, переменные и известные ограничения описаны в [руководстве по локальной разработке](docs/development/local-development.md).
