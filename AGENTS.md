...
Языковой слой TypeScript синтезирован в `proj-write-typescript` из [alleneubank/typescript-best-practices](https://skills.sh/alleneubank/claude-code/typescript-best-practices) и [Cursor typescript-best-practices](https://skills.sh/cursor/plugins/typescript-best-practices), а не установлен целиком: первый привязывает runtime validation к чужому `CLAUDE.md`, второй требует соседние `type-system-discipline` и `boundary-discipline` из большого plugin-пака. Остальные проверенные кандидаты отвергнуты по отдельным причинам: [sickn33/typescript-expert](https://skills.sh/sickn33/agentic-awesome-skills/typescript-expert) останавливает работу ради отсутствующих subagents и несёт широкий workflow; [LobeHub typescript](https://skills.sh/lobehub/lobehub/typescript) закрепляет React/UI и внутренние правила монорепозитория LobeHub; [jeffallan/typescript-pro](https://skills.sh/jeffallan/claude-skills/typescript-pro) навязывает advanced types, tRPC и type-coverage как baseline; [wshobson/typescript-advanced-types](https://skills.sh/wshobson/agents/typescript-advanced-types) — узкий справочник по type-level programming, а не повседневный Node.js skill. `codebase-design`, `tdd` и `writing-for-agents` уже покрыты tracked-клоном `mattpocock/_skills` и повторно не устанавливаются. Полезного grammY-specific skill или CLI для разработки не найдено, поэтому `proj-write-grammy-bot` выведен не из внешнего источника, а из [ADR-030](docs/decisions/ADR-030-telegram-bot.md) и [брифа компонента](docs/services/telegram-bot.md); Telegram MCP отложен до отдельного e2e-контура. Оба скилла держат примеры в `references/` и на нормативные правила ссылаются, а не пересказывают их.
