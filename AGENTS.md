...
- `apps/meetups/` — Meetups на F#: доменное ядро среза в `Domain/`, четыре команды записи в `Slices/`, доступ к PostgreSQL в `Infrastructure/`, gRPC-сервер с заглушечными ответами, C#-проект кодогенерации, миграции состояния сходки и журнала событий и два тестовых проекта; состояние и событие пишутся одной транзакцией, но связи транспорта с доменом ещё нет.
