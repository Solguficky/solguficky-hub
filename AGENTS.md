...
- Перед сдачей прогоняй `just verify` — механический гейт. Его состав перечислен один раз, в комментарии над рецептом `verify` в `justfile`; здесь и в других документах он не повторяется, потому что прозаические копии расходились с рецептом молча. Скилл `verify-this` решает другую задачу — проверяет отдельное утверждение экспериментом и гейт не заменяет.
