...
Tracked-клон приносит репозиторий целиком, поэтому лишнее гасится в `.skillignore`: из 46 скиллов пака Go включены 23, остальные выключены как ненужные этому репозиторию, а не как конфликтующие с нормативом. Клон и его скиллы в Git не лежат, а закоммиченные таргеты помечены в `.gitattributes` атрибутами `-diff` и `linguist-generated=true`: их содержимое проверяет `just check-agent-tools`, а не читатель диффа. Посмотреть правку скилла в таргете можно флагом `git diff --text`.
